 ...
 *
 * 첫·마지막 페이지는 ellipsis 와 별개로 항상 노출. 현재 페이지는 강조.
...
/**
 * 페이지 윈도우 계산.
 * - 첫 페이지·마지막 페이지는 항상 포함
 * - 현재 페이지 중심 좌우로 maxButtons 안에 들어가는 만큼
 * - 끊긴 구간은 "…" 마커
 *
 * 예: total=20, cur=10, max=7 → [1, …, 8, 9, 10, 11, 12, …, 20]
 */
function computeWindow(cur, total, maxButtons) {
  ...
  const side = Math.floor((maxButtons - 3) / 2); // 첫·마지막·현재 빼고 좌우
  let start = Math.max(2, cur - side);
  let end = Math.min(total - 1, cur + side);

  // 폭 보정 — start 또는 end 가 가장자리에 닿으면 반대편 늘리기
  if (cur - start < side) {
